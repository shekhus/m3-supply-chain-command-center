...
`STATUS: week 7 in progress — weeks 1–6 done (scaffold, generator matching Project A row for row, metric SQL + policy.yaml). B-4 done: three detectors (day-of-week adjusted baseline, policy thresholds, CUSUM) + severity with four gates; A1–A4/A6 reach HIGH in-window (lead 0/0/1/4/7 days), A5 the holiday decoy capped at WARN. B-5 done: exact rate/mix attribution ranked by rate effect — from the plant anomaly the top driver is the seeded lane C000031 (40.4% of the performance change), from the company total it is PLT-02, from PLT-01's fill rate it is CASE-READY; `make detect` prints drivers under each incident. 89 tests. Next: B-6 replay harness + scoring (`make replay`), where the detection floors get tuned against measured precision/recall rather than guessed. Week-7 DoD: replay over the full window scores recall ≥ 90%, precision ≥ 80%, top-1 attribution ≥ 85% against ground_truth/anomalies.json, with the A5 decoy not raised HIGH.`
