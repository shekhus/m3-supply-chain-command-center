...
`STATUS: week 7 in progress — weeks 1–6 done (B-1 scaffold, B-2 generator matching Project A row for row, B-3 metric SQL + policy.yaml + `make metrics`). B-4 done: three detectors (28-day day-of-week adjusted baseline, policy thresholds with consecutive-day rules, CUSUM change-point) + severity with four gates (thin volume, immaterial movement, no corroboration, holiday allowance); 11 watched grains over 173 segments; `make detect DATE=...`. A1–A4 and A6 reach HIGH in-window (lead 0/0/1/4/7 days), A5 the holiday decoy is reported but capped at WARN. 74 tests. Next: B-5 attribution (volume-weighted decomposition, top-3 drivers), then B-6 replay harness + scoring. Week-7 DoD: replay over the full window scores recall ≥ 90%, precision ≥ 80%, top-1 attribution ≥ 85% against ground_truth/anomalies.json, with the A5 decoy not raised HIGH.`
