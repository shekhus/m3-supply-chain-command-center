...
`STATUS: week 7 complete — weeks 1–6 done (scaffold, generator matching Project A row for row, metric SQL + policy.yaml). B-4 three detectors + severity gates; B-5 exact rate/mix attribution; B-6 replay harness + scoring (`make replay` → generated evals/REPLAY.md). Measured over 535 days: **recall 100% (5/5), precision 80%, attribution top-1 100%, median lead 1 day, holiday decoy raised HIGH 0 times** — 5 HIGH events in 18 months, 4 of them the seeded anomalies. The first replay read 2% precision and rewrote the policy: thresholds are now set from measured distributions (OTIF's old 0.90 sat at the median), scoped per grain, and HIGH means a policy line was crossed — statistics raise WARN. Backlog has no HIGH rule because the measurement showed one cannot work (A6 z=2.81 vs noise z=5.04). Floors tuned on 2025-03-29..2026-02-28; held-out half scores 2/2 and 100% precision. 111 tests. Next: week 8 — evidence pack (B-7), permission filter (B-8), narration prompt + JSON contract, validator + regenerate + templated fallback. Week-8 DoD: a 20-day narration sample passes the citation gate at 100% and the fallback path is tested.`
